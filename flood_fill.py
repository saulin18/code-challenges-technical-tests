...
# pixel value of the image. You are also given three integers sr, sc, and color. Your task is to perform a flood fill 
...
# (pixels that share a side with the original pixel, either horizontally or vertically) and shares 
...
def floodFill(self, image, sr, sc, color):
        ...
        def recurse(row, col, original_color):
            if row < 0 or row >= len(image) or col < 0 or col >= len(image[0]) or image[row][col] != original_color:
                return
            
            image[row][col] = color
            
            recurse(row + 1, col, original_color)
            recurse(row - 1, col, original_color)
            recurse(row, col + 1, original_color)
            recurse(row, col - 1, original_color)
