...
# 0
# 7
# All members of a groups (set) are conected to every other member If you asked is 0 connected to 7, it should return False
# If you asked is 8 connected to 9 you should return True even if 8,9 wern't directly joined. (8 is connected to 3 which is connected to 4
# which is connected to 9)
...
# Hint: There is a naive implenetation that takes time proportional to O(n^2) (Quadratic Funtion) This will time out the problem (Because every time
# you double the input like from 50 nodes to 100 nodes, the function takes 4 times as long) Test will check that you are able to outrun this naive i
# mplementation. If you can't that means that your code is too slow#
...
class DynamicConnectivity(object):
    ...
    def union(self, p, q):
        self.find(q)
        if (root1 := self.find(p)) != (root2 := self.find(q)):
            if self.rank[root1] < self.rank[root2]:
                self.parent[root1] = root2
            elif self.rank[root1] > self.rank[root2]:
                self.parent[root2] = root1
