...
def destCity(paths: list[list[str]]) -> str:
    ...
    def solve(paths):
        ...
        seen_cities = set()
        for path in paths:
            adj_list[path[0]].append(path[1]) if path[1] is not None else None
            seen_cities.add(path[0])
            seen_cities.add(path[1])
        return seen_cities.difference(adj_list.keys()).pop()
