...
class Solution:
    def isCovered(self, ranges: list[list[int]], left: int, right: int) -> bool:
        ...
        maximum_range = max(ranges[i][1] for i in range(len(ranges)))
        diff_array = [0] * (maximum_range + 2)
        for start, end in ranges: 
            diff_array[start] +=1
            diff_array[end + 1] -=1

        running_sum = 0
        for i in range(1 , maximum_range + 2):
            running_sum += diff_array[i]
            if running_sum <= 0 and i >= left and i <= right:
                return False
        return True