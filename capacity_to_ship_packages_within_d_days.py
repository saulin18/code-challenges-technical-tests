...
class Solution:
    def shipWithinDays(self, weights: List[int], days: int) -> int:
        minimum_weight_capacity = max(weights)
        maximum_weight_capacity = sum(weights)

        low = minimum_weight_capacity
        high = maximum_weight_capacity
        ans = float("inf")

        while low < high:
            middle = low + (high - low) // 2

            number_of_days_needed = self.calculate_number_of_days_needed(
                weights, middle
            )
    ...
    def calculate_number_of_days_needed(
        self,
        weights: List[int],
        weight_capacity: int,
    ) -> int:
        number_of_days_needed = 0
        current_weight_capacity = 0
        for weight in weights:
            if current_weight_capacity + weight > weight_capacity:
                number_of_days_needed += 1
                current_weight_capacity = weight
            else:
                current_weight_capacity += weight

        return number_of_days_needed + 1
