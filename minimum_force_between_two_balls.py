...
class Solution:
    def maxDistance(self, position: List[int], m: int) -> int:
        ...
        left = 0
        right = max(position)
        sorted_forces = sorted(position)

        def can(force: int):
            last_ball = sorted_forces[0]
            num_balls = 1
            for i in range(1, len(sorted_forces)):
                if sorted_forces[i] - last_ball >= force:
                    num_balls += 1
                    last_ball = sorted_forces[i]

            return False if num_balls < m else True
