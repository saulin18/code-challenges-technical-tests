...
# You are given a 1-indexed integer array numWays, where numWays[i] represents the number of ways to select a
# total amount i using an infinite supply of some fixed coin denominations. Each denomination is a positive integer
# with value at most numWays.length.
...
def findCoins(self, numWays: List[int]) -> List[int]:
    ...
    for sum in range(1, len(numWays) + 1):
    
        if numWays[sum - 1] > dp[sum]:
            coins.append(sum)
            for currentSum in range(sum, len(numWays) + 1):
                dp[currentSum] += dp[currentSum - sum]
        if dp[sum] != numWays[sum - 1]:
            return []
