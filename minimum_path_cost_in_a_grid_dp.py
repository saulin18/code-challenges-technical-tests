...
# You are given a 0-indexed m x n integer matrix grid consisting of distinct integers from 0
# to m * n - 1. You can move in this matrix from a cell to any other cell in the next row.
# That is, if you are in cell (x, y) such that x < m - 1, you can move to any of the cells
# (x + 1, 0), (x + 1, 1), ..., (x + 1, n - 1). Note that it is not possible to move from cells in the last row.
...
# of all the moves made. Return the minimum cost of a path that starts from any cell in
# the first row and ends at any cell in the last row.
        ...
        for i in range(1, m):
            for j in range(n):
                ...
                for prev_j in range(n):
                    cost = dp[i - 1][prev_j] + moveCost[grid[i - 1][prev_j]][j]
                    cost += grid[i][j]
