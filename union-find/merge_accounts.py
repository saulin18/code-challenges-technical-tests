...
# have the same name. A person can have any number of accounts initially,
...
# Output: [["Ethan","Ethan0@m.co","Ethan4@m.co","Ethan5@m.co"],["Gabe","Gabe0@m.co",
        ...
        for i, a in enumerate(accounts):
            for email in a[1:]:
                if email in email_to_account_index:
                    uf.union(i, email_to_account_index[email])
                else:
                    email_to_account_index[email] = i

        email_grouped = defaultdict(list)
        for email, account_index in email_to_account_index.items():
            email_grouped[uf.find(account_index)].append(email)
        return [
            [accounts[index][0]] + sorted(emails)
            for index, emails in email_grouped.items()
        ]
